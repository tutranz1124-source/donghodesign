      ...
      res.on('end', async () => {
        try {
          ...
          ws.onopen = async () => {
            ...
            await send('Runtime.evaluate', { expression: 'window.scrollBy(0, 3600)' });
            await new Promise(r => setTimeout(r, 1500));
            const shot = await send('Page.captureScreenshot', { format: 'png' });
            const brainDir = 'C:\\Users\\kaizk\\.gemini\\antigravity\\brain\\5ee9230c-8a97-48c7-9f2e-c223c1fd9e5a';
            fs.writeFileSync(path.join(brainDir, 'verify_public_blog_detail_related.png'), Buffer.from(shot.data, 'base64'));
            console.log('Saved verify_public_blog_detail_related.png');
            ws.close();
            chrome.kill();
            resolve();
          };
